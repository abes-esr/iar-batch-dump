...
WITH notice_candidate AS (
    SELECT
        b.ppn,
        (select ppn from biblio_table_frbr_1XX a where b.ppn=a.ppn and a.tag='105$a' and (substr(a.datas,4,4) like '%v%' or substr(a.datas,4,4) like '%m%' or substr(a.datas,4,4) like '%7%') and  rownum<2) as ppn_these
...
JOIN biblio_table_frbr_6XX a ON a.ppn = c.ppn
JOIN biblio_table_frbr_6XX b ON b.ppn = a.ppn AND b.posfield = a.posfield
WHERE a.tag = '606$2' AND a.datas = 'rameau'

  -- La zone 606 possède un identifiant RAMEAU ($3)
  AND b.tag = '606$3'

  AND EXISTS ( SELECT 1 FROM aut_table_frbr_0xx g WHERE g.ppn = b.datas AND g.tag = '008$0' AND g.datas = 'Td8' )  -- Le concept RAMEAU associé est de type Td8
  AND NOT EXISTS ( SELECT 1 FROM aut_table_frbr_9XX f WHERE f.ppn = b.datas AND f.tag = '950$a' AND f.datas LIKE '%--%' )   -- Le concept n'est pas un RAMEAU imbriqué (?)
),
...
rameau_par_zone AS (
    ...
    JOIN biblio_table_frbr_6XX zz ON zz.ppn = v.ppn AND zz.posfield = v.posfield AND zz.tag IN ('606$a', '606$x')
    JOIN biblio_table_frbr_6XX zz2 ON zz2.ppn = v.ppn AND zz2.posfield = v.posfield AND zz.POSSUBFIELD - 1 = zz2.POSSUBFIELD AND zz2.tag = '606$3'
...
rameau_par_notice AS (
    SELECT
        ppn,
        RTRIM( XMLAGG( XMLELEMENT( e, rameau || ';' ) ORDER BY posfield ).EXTRACT('//text()').GETCLOBVAL(), ';') AS rameau
